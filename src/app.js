document.addEventListener('DOMContentLoaded', function () {
	...
	const coins = {
		USD: [25, 10, 5, 1], // quarter, dime, nickel, penny
		GBP: [200, 100, 50, 20, 10, 5, 2, 1], // £2, £1, 50p, 20p, 10p, 5p, 2p, 1p
		EUR: [200, 100, 50, 20, 10, 5, 2, 1]  // €2, €1, 50c, 20c, 10c, 5c, 2c, 1c
	};
	...
	form.addEventListener('submit', async function (e) {
		...
		for (let i = 0; i < coins[currency].length; i++) {
			const count = Math.floor(remaining / coins[currency][i]);
			breakdown.push(count);
			remaining -= count * coins[currency][i];
		}

		// Output
		let output = `<strong>Converted Amount:</strong> ${smallestUnit} ${(currency === 'USD') ? 'cents' : (currency === 'GBP') ? 'pence' : 'euro cents'}<br>`;
		output += `<strong>Exchange Rate (USD → ${currency}):</strong> ${rate.toFixed(4)}<br>`;
		output += `<strong>Date:</strong> ${date}<br><br>`;
		output += '<strong>Coin Breakdown:</strong><ul>';
		breakdown.forEach((count, i) => {
			output += `<li>${coinNames[currency][i]}: ${count}</li>`;
		});
		output += '</ul>';
